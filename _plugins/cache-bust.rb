...
module Jekyll
    module CacheBust
        ...
        def bust_css_cache(file_name)
            css_sources = Dir['_sass/**/*'].select { |path| File.file?(path) }
            css_sources << 'assets/css/main.scss'
            digest = Digest::MD5.hexdigest(css_sources.sort.map { |path| File.read(path) }.join)
